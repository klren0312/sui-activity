...
import { Badge, Card, Carousel, Button, Tag, Descriptions, Modal, message, Image } from 'antd'
...
export default function ActivityCard({ activity }: ActivityCardProps) {
  ...
  return (
    <>
      {contextHolder}
      <Badge.Ribbon
        color={parseInt(activity.join_fee) === 0 ? 'green' : 'volcano'}
        text={
          <div>
            <div>{activity.tag + (parseInt(activity.join_fee) === 0 ? '（免费）' : '（收费）')}</div>
            <div>{activity.date_range[0]} - {activity.date_range[1]}</div>
          </div>
        }
      >
        {/* 活动卡片 */}
